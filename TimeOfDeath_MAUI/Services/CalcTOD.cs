...
{
    public class CalcTOD : ICalcTOD
    {
        double[,] correctionData = new double[3, 20];
        double[,] iterations = new double[3, 20];
        double c = 0, b = 0, cas = 0;

        BaseViewModel? vm => Startup.ServiceProvider.GetService<BaseViewModel>();
        ...
        private double CorrectionFactor
        {
            ...
            {
                double correction = 0;
                switch (vm.PartClothed)
                {
                    case 0: // dry naked
                        correction = vm.AirTemp == 0 ? 0.75 : 1;
                        break;
                    case 1: // dry 1-2 thin layers
                        correction = vm.AirTemp == 0 ? 0.9 : 1.1;
                        break;
                    ...
                    case 3: // dry 3-4 thin layers
                        ...
                        break;
                    case 5: // more layers
                        correction = vm.AirTemp == 2 ? 1.8 : 1.4;
                        break;
                    case 6: // naked
                        if (vm.Water != 2 && vm.AirTemp == -2)
                            correction = vm.Water == 0 ? 0.35 : 0.5;
                        else
                            correction = 0.7;
                        break;
                    ...
                    case 9: // wet, 2 or more
                        correction = vm.PartInAir == 0 ? 1.2 : 0.9;
                        break;
                }
                return correction;
            }
        ...
        public DateTime CalcTimeOfDeath(DateTime death)
        {
            double ta = vm.SurroundTemp, tr = vm.BodyTemperature, m = vm.BodyWeight;
            ...
            vm.DateOfDeath = todead;
            vm.TimeOfDeath = todead.TimeOfDay;
            return todead;
        }
    }
}
